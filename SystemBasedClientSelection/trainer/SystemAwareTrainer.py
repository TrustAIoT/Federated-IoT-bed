...
class SystemAwareTrainer(ClientTrainer):
    ...
    def _init_device_query(self):
        self.communicator_stats = SystemInfoCommunication(args.mqtt_broker, 1883, 
                                                    args.broker_username, args.broker_password,
                                                    self.args.rank, self.client_real_ids,
                                                    topic="stats", 
                                                    _on_message_callback=self.message_callback)
        self.deviceQuery = DeviceInfo(self.args, self.comp_callback, self.network_callback)

    def message_callback(self, topic, payload):
        server_id = topic.split("/")[-1]
        fedml.logging.info("Received from server: {}, message: {}.".format(server_id, payload))
