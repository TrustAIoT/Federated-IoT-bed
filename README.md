# FLINT IoT Testbed: Federated and Distributed Learning on Heterogeneous Devices
*(From NSF CNS Core: Small: "FLINT: Robust Federated Learning for Internet of Things”)*

## 1 About the Project 

This repository documents the design and implementation of the **FLINT wireless mesh IoT testbed**, developed as part of the NSF project *CNS Core: Small: Robust Federated Learning for Internet of Things (FLINT)*.

The rapid growth of IoT devices has created vast opportunities for **federated and distributed machine learning (FDML)** on the edge. However, much of the current research in this space relies heavily on simulation, which often overlooks key challenges such as **system heterogeneity, communication overhead, and real-world networking dynamics**.

To bridge this reality gap, we built a physical IoT testbed that integrates heterogeneous devices — **Raspberry Pi 4 Model B, Jetson Nano, and Jetson Orin Nano** — connected via a **mesh ad-hoc network** using the **B.A.T.M.A.N IV routing protocol**. Our testbed enables empirical studies on both networking and federated learning performance under realistic deployment conditions.

As an initial validation, we trained a ResNet-18 model on the CIFAR-10 dataset using the **FedAvg algorithm** across multiple devices. Our experiments highlight critical challenges, including increased training times due to limited compute on IoT devices and significant communication delays in multi-hop networks.

The FLINT testbed thus provides a **research and education platform** for exploring robust federated learning in heterogeneous IoT environments, complementing and extending simulation-based studies.
...
https://github.com/TrustAIoT/FedML-BATMAN/blob/main/BATMAN.md
...
**Initializing the Application**: Setting up necessary configurations, initializing variables, and preparing the environment.

**Running Core Functionalities**: Executing the main functions of the application, such as processing data, running algorithms, or managing user interactions.

**Orchestrating Other Components**: Calling functions or methods from other modules and managing the workflow of the application.

**Handling Inputs and Outputs**: Managing input from users or other systems and producing the required outputs, whether they are files, visualizations, or other forms of data.
