########################################################
# Retrieved from fedml.data.cifar10.efficient_loader.py
########################################################
...
def partition_data(dataset, datadir, partition, n_nets, alpha, process_id, synthetic_data_url, private_local_data, client_proportions=None):
    np.random.seed(10)
...
########################################################
# Addition for splitting data among clients with iid label and data samples
########################################################
        ...
        for k in range(K):
            ...
            idxs = np.random.shuffle(idx_k)
            batch_idxs = np.array_split(idxs, n_nets)
            for j in range(n_nets):
                idx_batch[j] += batch_idxs[j].tolist()
        for j in range(n_nets):
...
########################################################
# Addition for splitting data among clients with proportions
########################################################
    elif partition == "hetero-proportional":
        min_size = 0
        K = 10
        N = y_train.shape[0]
        logging.info("N = " + str(N))
        ...
        while min_size < 10:
            ...
            for k in range(K):
                ...
                proportions = np.random.dirichlet(np.repeat(alpha, n_nets))
                ## Balance
                proportions = np.array([p * (len(idx_j) < sample_size) 
                                        for p, idx_j, sample_size 
                                        in zip(proportions, idx_batch, N * np.array(client_proportions))])
                proportions = proportions / proportions.sum()
                proportions = (np.cumsum(proportions) * len(idx_k)).astype(int)[:-1]
                idx_batch = [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k, proportions))]
                min_size = min([len(idx_j) for idx_j in idx_batch])
...
def efficient_load_partition_data_cifar10(
    ...
    (
        ...
        cifar10_test_ds,
                       ...
                       process_id, synthetic_data_url, private_local_data, client_proportions)
